...
export function SceneLighting() {
  return (
    <>
      ...
      <ambientLight color="#33261a" intensity={0.4} />

      {/* Soft warm key, front-upper-left — patined beige, never white. */}
      <directionalLight color="#b29e83" intensity={1.15} position={[-4, 6, 8]} />

      {/* Warm rim from below-right (Seal Brown) for depth. */}
      <pointLight color="#553927" intensity={9} distance={40} decay={2} position={[6, -4, -6]} />

      {/* Very faint front bounce so the near fibres never go pitch black. */}
      <pointLight color="#826a51" intensity={3} distance={28} decay={2} position={[3, 2, 6]} />

      {/* Back rim (spec: "back light for separation") — detaches the dark
          membranes from the dark backdrop without brightening the scene. */}
      <directionalLight color="#826a51" intensity={0.35} position={[0, 3, -8]} />
    </>
  );
}
